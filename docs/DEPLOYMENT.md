...
Production base path on Pages is `/enchanted-forest/`; Vite's
`vite.config.ts` sets that from `GITHUB_PAGES=true`.
...
Without these the release job produces an unsigned debug AAB.
